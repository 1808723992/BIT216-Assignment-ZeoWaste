...
if ($leftNum <= 0) {
    ...
    $upd = $conn->prepare("UPDATE fooditems SET food_status='completed', completed_at=NOW(), food_quantity='0', updated_at=NOW() WHERE food_id=? AND user_id=?");
    $upd->bind_param('ii', $food_id, $user_id);
    $ok = $upd->execute();
    if (!$ok) { echo json_encode(["success"=>false, "error"=>$upd->error]); exit; }
