...
function getAPIBase() {
  ...
  // 如果已经在子目录中，需要回到根目录
  // 例如：/BIT216-Assignment-ZeoWaste/Main/ManageInventory/xxx.html -> /BIT216-Assignment-ZeoWaste/Main/
  // 或者：/Main/Manage Food Inventory_GUO.html -> /Main/
  const apiPath = baseDir + 'ManageInventory/api/api.php';
  
  console.log('[getAPIBase] Current path:', currentPath);
  console.log('[getAPIBase] Base dir:', baseDir);
  console.log('[getAPIBase] API path:', apiPath);
...
async function api(action, method='GET', body=null, params=null){
  ...
  console.log(`[API] ${method} ${action}`, { url: url.toString(), body, params });
  ...
  try {
    ...
    console.log(`[API] Response for ${action}:`, { status: res.status, data });
    
    if (res.status === 401 || data.error === 'UNAUTHORIZED') {
      console.error('[API] Authentication failed, redirecting to login page');
...
const ALLOWED_CATEGORIES = ['Dairy','Vegetable','Bakery','Grains','Meat','Fruits'];
...
async function loadActive(){
  ...
  console.log('[Load Active] Fetching with params:', params);
  const {items} = await api('list_items','GET',null,params);
  cacheActive = items || [];
  console.log('[Load Active] Loaded', cacheActive.length, 'items');
}
...
foodForm.addEventListener('submit', async (e)=>{
  ...
  if(!name || !quantity || !expiry_date || !category || !storage){ 
    showToast('Please complete all required fields.','error'); 
    return; 
  }

  console.log('[Add Item] Submitting:', { name, quantity, expiry_date, category, storage, notes });

  try{
    const res = await api('add_item','POST',{ name, quantity, expiry_date, category, storage, notes });
    console.log('[Add Item] Success:', res);
    showToast(res.merged ? 'Quantity merged into existing item.' : 'Item added successfully.');
    foodForm.reset();
    
    // 强制刷新列表
    console.log('[Add Item] Refreshing inventory...');
    await refreshAll();
    console.log('[Add Item] Refresh completed');
  }catch(e){
    console.error('[Add Item] Error:', e);
    showToast(`Failed to add item: ${e.message || 'Unknown error'}`, 'error');
  }
});
...
async function onMarkUsed(e){
  const id = e.currentTarget.getAttribute('data-id');
  const input = prompt('Enter quantity used (e.g., 1, 2kg, 1 bottle):','1');
...
// Close donation modal on Cancel
