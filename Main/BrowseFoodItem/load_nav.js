...
(function () {
  // 计算脚本所在目录作为基准路径（兼容被不同页面引用的情况）
  function getScriptBase(){
    const script = document.currentScript || Array.from(document.getElementsByTagName('script')).find(s => (s.src||'').endsWith('load_nav.js'));
    if (!script) return '';
    const url = new URL(script.src, window.location.origin);
    url.pathname = url.pathname.replace(/\\/g,'/');
    return url.pathname.substring(0, url.pathname.lastIndexOf('/') + 1); // 以 / 结尾
  }
  ...
  fetch(BASE + "nav.html")
    ...
    .then(html => {
      ...
      // 自动加载 nav.css（相对脚本所在目录）
      if (!document.querySelector(`link[href="${BASE}nav.css"]`)) {
